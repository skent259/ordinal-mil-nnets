...
class RReLu(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        alpha = K.in_train_phase(
            K.random_uniform(inputs.shape[1:], 0.0, 1.0),
...
class PELU(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        pos = (
            cons_greater_zero(self.alpha) / cons_greater_zero(self.beta)
        ) * keras.activations.relu(inputs)
...
class CLM(keras.layers.Layer):
    ...
    def _convert_thresholds(self, b, a):
        ...
        th = K.sum(
            MatrixBandPart(K.ones([self.num_classes - 1, self.num_classes - 1]), -1, 0)
            * K.reshape(
                K.tile(thresholds_param, [self.num_classes - 1]),
    ...
    def _nnpom(self, projected, thresholds):
        ...
        if self.link_function == "probit":
            ...
            a3T = K.cond(
                self.q < 0,
                lambda: igammac(
                    K.pow(self.q, -2), K.pow(self.q, -2) * K.exp(self.q * z3)
                ),
                lambda: K.cond(
                    self.q > 0,
                    lambda: igamma(
    ...
    def build(self, input_shape):
        ...
        if self.link_function == "glogit":
            ...
            if not "r" in self.p:
                self.p["r"] = self.add_weight(
                    "r_nnpom", shape=(1,), initializer=keras.initializers.Constant(1.0)
                )
                self.p["r"] = K.clip(self.p["r"], 0.05, 100)

            if not "mu" in self.p:
                self.p["mu"] = self.add_weight(
                    "mu_nnpom", shape=(1,), initializer=keras.initializers.Constant(0.0)
                )
            ...
            self.sigma = self.add_weight(
                "sigma_nnpom", shape=(1,), initializer=keras.initializers.Constant(1.0)
            )
            self.lmbd = self.add_weight(
                "lambda_nnpom", shape=(1,), initializer=keras.initializers.Constant(1.0)
            )
        elif self.link_function == "ggamma":
            self.__set_default_param(
                "a",
                self.add_weight(
                    "a_clm", shape=(1,), initializer=keras.initializers.Constant(0.5)
                ),
            )
            self.__set_default_param(
                "d",
                self.add_weight(
                    "d_clm", shape=(1,), initializer=keras.initializers.Constant(0.5)
                ),
            )
            self.__set_default_param(
                "p",
                self.add_weight(
                    "p_clm", shape=(1,), initializer=keras.initializers.Constant(0.5)
                ),
            )
