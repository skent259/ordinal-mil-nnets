"""
File copied from https://github.com/ayrna/deep-ordinal-clm/blob/master/src/losses.py
2022-06-12
"""
import numpy as np
...
def make_cost_matrix(num_ratings):
    ...
    cost_matrix = (
        np.power(cost_matrix - np.transpose(cost_matrix), 2) / (num_ratings - 1) ** 2.0
    )
...
def qwk_loss(cost_matrix):
    ...
    def _qwk_loss(true_prob, pred_prob):
        ...
        a = K.reshape(
            K.dot(cost_matrix, K.reshape(sum_prob, shape=[-1, 1])), shape=[-1]
        )
        ...
        )  # + K.cast(K.sum(conf_mat) * 0, dtype=K.floatx())
