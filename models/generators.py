...
class MILImageDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        y_batch = np.asarray([self.__get_output(i) for i in label_batch])
...
class MILTextDataGenerator(MILImageDataGenerator):
    """
    Keras Sequence to pull in batches of bags in a MIL data set
    ...
    def __get_input(self, text):
        """Break text up into separate sentences"""
        sentences = nltk.tokenize.sent_tokenize(text)  # List[str]
    ...
    def __getitem__(self, index):
        ...
        text_batch = batches[self.x_col]
        label_batch = batches[self.y_col]

        # Pull text and combine as array for each bag
        X_batch = np.asarray([self.__get_input(i) for i in text_batch])
        y_batch = np.asarray([self.__get_output(i) for i in label_batch])
