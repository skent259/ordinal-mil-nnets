...
#| fig-height: 8
plot_two_factors(df_metrics_o, exp_list, metric = "acc", facets = "data_set_type", nrow=1) +
  labs(color = NULL)
```
...
walk2(
  models$fit, models$outcome,
  function(.x, .y) {
    out <- emmeans(.x, list(pairwise ~ ordinal_method), adjust = "tukey", data = models$df[[1]])
    
    lev <- out[[1]] %>% as_tibble() %>% arrange(emmean) %>% pull(1)
    p <- plot(out, comparisons=TRUE) + 
...
walk2(
  models$fit, models$outcome,
  function(.x, .y) {
    out <- emmeans(.x, list(pairwise ~ mil_pool_combo), adjust = "tukey", data = models$df[[1]])
