...
exp_list <- c("fgnet-1.0.1", "fgnet-1.0.2", "fgnet-1.0.3", "fgnet-1.0.4", "bcnb-3.0.1", "bcnb-3.0.2", "amrev-4.0.1", "imdb-5.0.1", "afad-6.0.1")
...
Formula used: 
``` r
metric ~ (ordinal_method + mil_pool_combo + data_set_type)^3 + (1 | data_set_name) + learning_rate
```
...
models <- 
  ...
  mutate(
    ...
    emmean_or_tukey = list(emmeans(fit, list(pairwise ~ ordinal_method), adjust = "tukey", data = df)),
    emmean_mil_tukey = list(emmeans(fit, list(pairwise ~ mil_pool_combo), adjust = "tukey", data = df))
  )

saveRDS(models, here("analysis/objects/models-01.rds"))
...
#| fig-cap: Main single factor effects with MZOE outcome
#| fig-height: 8
#| fig-width: 12
plot_one_factor(df_metrics, exp_list, "ordinal_method", "mzoe", facets = "data_set_type") + 
  plot_one_factor(df_metrics, exp_list, "mil_pool_combo", "mzoe", facets = "data_set_type") +
  plot_layout(ncol = 1, heights = c(1, 2))
```
...
#| fig-height: 8
plot_two_factors(df_metrics_o, exp_list, metric = "mae", facets = "data_set_type", nrow=1) +
  labs(color = NULL)
```

```{r}
#| label: fig-2f-boxplot-acc-mil_ordinal
#| fig-cap: Two factor effects of MIL and Ordinal method with MZOE outcome. 
#| fig-height: 8
plot_two_factors(df_metrics_o, exp_list, metric = "mzoe", facets = "data_set_type", nrow=1) +
  labs(color = NULL)
```

Note that for IMDB, the results flip---low MAE usually corresponds to low accuracy. The others are largely consistent.
...
For MAE and RMSE, we can distinguish that CORN is better than CORAL averaged over MIL and Data Set levels (and ignoring interactions). For Accuracy, we cannot make the distinction although on average the emmean for CORN is lower than CORAL. Surprisingly, CLM_QWK_CLOGLOG is competitive in terms of Accuracy with CORN and CORAL (probably due to high accuracy in the BCNB and IMDB settings). 

```{r}
#| message: false

walk2(
  models$emmean_or_tukey, models$outcome,
  function(.x, .y) {
    
    lev <- .x[[1]] %>% as_tibble() %>% arrange(emmean) %>% pull(1)
    p <- plot(.x, comparisons=TRUE) + 
      scale_y_discrete(limits = rev(lev)) +
      labs(
        title = glue("Tukey EM mean plot for {str_to_upper(.y)}")
      ) +
      theme_light()
...
walk2(
  models$emmean_mil_tukey, models$outcome,
  function(.x, .y) {
...
plot_training_progression <- function(df, exps, metric, group, alpha = 0.1) {
  ...
  df %>% 
    ...
    theme(legend.position = "bottom") +
    labs(
      title = glue("Training: {glue_collapse(exps, ', ')}")
    )
}
```
...
# p <- df_train_prog %>% 
#   filter(ordinal_method %in% c("CORN")) %>%
#   plot_training_progression(
#     c("imdb-5.0.1"), 
#     val_loss, 
#     group = mil_pool_combo
...
exp_list <- c("fgnet-1.0.1", "fgnet-1.0.2", "fgnet-1.0.3", "fgnet-1.0.4", "bcnb-3.0.1", "bcnb-3.0.2", "amrev-4.0.1", "imdb-5.0.1", "afad-6.0.1")
df_metrics %>% 
  # filter(ordinal_method %in% c("CORN", "CORAL")) %>% 
  filter(experiment %in% exp_list) %>% 
...
**Why do the results flip for the IMDB results?** While not universally true, the trends below show that most of the high accuracy models have a small number of predictions, and likely aren't the best models to use. 

```{r}
p <- df_metrics %>% 
  filter(experiment %in% exp_list) %>% 
  ggplot(aes(acc, mae, color = n_pred)) + 
  geom_point(alpha = 0.5) +
  facet_wrap(~data_set_type) +
  scale_color_viridis_c(option = "magma") + 
  theme_bw()

print(p)
p %+% filter(df_metrics, experiment == "imdb-5.0.1") 
