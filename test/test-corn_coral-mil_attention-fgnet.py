...
from models.mil_attention.layer import MILAttentionLayer, mil_attention_layer
...
# Model 1: MI-net with softmax output (categorical)
inputs = Input(shape=(None,) + ds.params["img_size"])

x = BagWise(Conv2D(32, (5, 5), padding="same", activation="relu"))(inputs)
x = BagWise(Conv2D(32, (5, 5), activation="relu"))(x)
x = BagWise(MaxPooling2D(pool_size=(3, 3)))(x)
x = Dropout(0.25)(x)

x = BagWise(Conv2D(64, (3, 3), padding="same", activation="relu"))(x)
x = BagWise(Conv2D(64, (3, 3), activation="relu"))(x)
x = BagWise(MaxPooling2D(pool_size=(3, 3)))(x)
x = Dropout(0.25)(x)
...
x = BagWise(Flatten())(x)
x = BagWise(Dense(256, activation="relu"))(x)
x = Dropout(0.5)(x)

outputs = mil_attention_layer(
    x, 128, use_gated=False, kernel_regularizer=tf.keras.regularizers.l2(0.01)
)
# outputs = coral.CoralOrdinal(n_classes)(outputs)
outputs = coral.CornOrdinal(n_classes)(outputs)

model = tf.keras.Model(inputs=inputs, outputs=outputs, name="MI-net_corn_convnet")
model.compile(
